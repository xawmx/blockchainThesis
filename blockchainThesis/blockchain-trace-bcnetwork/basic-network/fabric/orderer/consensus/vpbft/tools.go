...
// Get nodes with low PR value
func (op *obcBatch) getLowPRValueNodes() []uint64 {
	// 将 map 转换为切片
	type kv struct {
		Key   uint64
		Value float64
	...
	op.pbft.graph.RankWithDefault(func(id uint64, rank float64) {
		ss = append(ss, kv{id, rank})
	})
...
func (op *obcBatch) sendVRF() {
	...
	if op.pbft.ifSendVrf == false {
		...
		lenBpk := []byte{}
		binary.BigEndian.PutUint16(lenBpk, uint16(len(bpk)))
		data = append(data, lenBpk...)
		data = append(data, bpk...)

		nodes := op.getLowPRValueNodes()
		byteArr := make([]byte, len(nodes)*8)
		for i, v := range nodes {
			binary.BigEndian.PutUint64(byteArr[i*8:], v)
		}
...
func (op *obcBatch) verifyVRF(event events.Event) events.Event {
	...
	bpklength := int(binary.BigEndian.Uint16(data[35+plength : 37+plength]))
	bpk := []byte(data[37+plength : 37+plength+bpklength])
	pk, _ := PEMToPublicKey(bpk)
	lowPRValueNodes := []byte(data[37+plength+bpklength:])
	nodes := make([]uint64, len(lowPRValueNodes)/8)
	for i := 0; i < len(nodes); i++ {
		nodes[i] = binary.BigEndian.Uint64(lowPRValueNodes[i*8:])
	}

	if op.pbft.lowTrustPeers == nil {
		op.pbft.lowTrustPeers = make(map[uint64]uint64)
	}
		...
		if r == false {
			...
			if flag == 0 {
				...
				for _, node := range nodes {
					if value, ok := op.pbft.lowTrustPeers[node]; ok {
						op.pbft.lowTrustPeers[node] = uint64(data[0])
					} else {
						op.pbft.lowTrustPeers[node] = value + 1
					}
				}
			}
		}
...
func (op *obcBatch) selectLeader(choice int) {
	...
	//}
	for i := 0; i < len(op.pbft.vrfs)-1; i++ {
		tmpLeader := op.pbft.vrf_peers[op.pbft.vrfs[0]] + uint64(1)
		if value, ok := op.pbft.lowTrustPeers[tmpLeader]; !ok || int(value) < op.pbft.N/3 {
			//vrf依次排序且大于 2N/3 的节点信任的是主节点
			op.pbft.leader = op.pbft.vrf_peers[op.pbft.vrfs[i]] + uint64(1)
		}
	}
	//找f+1个clerk
...
func (instance *pbftCore) resetPeerWeight(e activeDetectimeoutEvent) {
	weight := instance.graph.GetLinkWeight(instance.id, e.id)

	weight -= resetWeight
	if weight <= resetWeight {
		weight = resetWeight
	}

	instance.graph.ResetLinkWeight(instance.id, e.id, weight)

	for i, peer := range instance.peers {
		if peer == e.id {
			instance.activeDetectimers[i].Reset(instance.activeDetectimeout, activeDetectimeoutEvent{e.id})
		}
	}
}
