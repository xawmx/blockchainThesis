...
func New(stack consensus.Stack) consensus.Consenter {
	handle, peers, _ := stack.GetNetworkHandles()
	id, _ := getValidatorID(handle)
	peerIDs := getPeersIDs(peers)

	switch strings.ToLower(config.GetString("general.mode")) {
	case "batch":
		return newObcBatch(id, peerIDs, config, stack)
	default:
		panic(fmt.Errorf("Invalid PBFT mode: %s", config.GetString("general.mode")))
	}
}

func getPeersIDs(peers []*pb.PeerID) []uint64 {
	var ids []uint64
	for _, p := range peers {
		id, _ := getValidatorID(p)
		ids = append(ids, id)
	}
	return ids
}
...
func loadConfig() (config *viper.Viper) {
	...
	for _, p := range filepath.SplitList(gopath) {
		pbftpath := filepath.Join(p, "src/github.com/cherryFloris/pbft-vrf/consensus/pbft")
...
func getValidatorID(handle *pb.PeerID) (id uint64, err error) {
	// as requested here: https://github.com/cherryFloris/pbft-vrf/issues/462#issuecomment-170785410
