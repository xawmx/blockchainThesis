...
//Queue()接口返回一个类型为Event的channel，用于存储事件。Fabric用一个队列来存储事件，Queue()返回该channel对象。
//Start()方法会启动一个goroutine循环处理接收到的事件，通过channel能够保证只有接收到事件才会处理，不用每时每刻循环检查队列去执行事件，浪费CPU性能。
//SetReceiver(Recevier)需要设置事件管理器的实际处理者，Receiver接口需要实现ProcessEvent(Event) Event方法。
//Inject(Event)会在Queue()收到事件之后将事件发放给Receiver进行处理
//Halt()停止Manager线程
...
type Manager interface {
	...
	Start()               // 启动Manager线程 TODO, these thread management things should probably go away
	Halt()                // 停止Manager线程
}
