...
import (
	...
	cutil "github.com/cherryFloris/pbft-vrf/core/container/util"
...
func writeChaincodePackage(spec *pb.ChaincodeSpec, tw *tar.Writer) error {
	...
	newRunLine := fmt.Sprintf("RUN go install %s && cp src/github.com/cherryFloris/pbft-vrf/peer/core.yaml $GOPATH/bin && mv $GOPATH/bin/%s $GOPATH/bin/%s", urlLocation, chaincodeGoName, spec.ChaincodeID.Name)
