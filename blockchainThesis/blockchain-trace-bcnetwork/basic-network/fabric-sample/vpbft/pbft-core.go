...
import (
	...
	_ "github.com/cherryFloris/pbft-vrf/core" // Needed for logging format init
...
type activeDetectimeoutEvent struct {
	id uint64
}
...
type pbftCore struct {
	...
	peers              []uint64
	lowTrustPeers      map[uint64]uint64
	graph              *Graph         // pagerank graph
	activeDetectimers  []events.Timer // detect timer for active nodes
	activeDetectimeout time.Duration  // timeout for detecting active nodes
}
...
// constructors
// =============================================================================
func instancePbftCore(id uint64, peers []uint64, config *viper.Viper, consumer innerStack, etf events.TimerFactory) *pbftCore {
	var err error
	instance := &pbftCore{}
	instance.id = id
	...
	instance.lowTrustPeers = make(map[uint64]uint64)
	...
	instance.peers = peersIDs
	instance.graph = newPbftPageRankGraph(id, peers)
	instance.activeDetectimeout, err = time.ParseDuration(config.GetString("general.timeout.detect"))
	if instance.activeDetectimeout == 0 {
		instance.activeDetectimeout = activeDetectimeout
	}
	instance.activeDetectimers = newDetectimer(peers, etf)

	return instance
}

func newDetectimer(peers []uint64, etf events.TimerFactory) []events.Timer {
	var detectimers []events.Timer

	for _, _ = range peers {
		t := etf.CreateTimer()
		detectimers = append(detectimers, t)
	}
...
func newPbftCore(id uint64, config *viper.Viper, consumer innerStack, etf events.TimerFactory) *pbftCore {
	return instancePbftCore(id, []uint64{}, config, consumer, etf)
}

func newPbftCoreWithPeers(id uint64, peers []uint64, config *viper.Viper, consumer innerStack, etf events.TimerFactory) *pbftCore {
	return instancePbftCore(id, peers, config, consumer, etf)
}

func newPbftPageRankGraph(id uint64, peers []uint64) *Graph {
	graph := NewGraph()

	for _, peer := range peers {
		graph.Link(id, peer, weight)
	}
...
func (instance *pbftCore) ProcessEvent(e events.Event) events.Event {
	...
	instance.processPbftEventForDetectimer(e)
	...
	case waitTimeoutEvent:
		...
		instance.sendVRF2()
	case activeDetectimeoutEvent:
		instance.resetPeerWeight(et)
...
func (instance *pbftCore) recvPrepare(prep *Prepare) error {
	...
	instance.lowTrustPeers = map[uint64]uint64{}
	instance.leaderCheatingNum = 0
	instance.pdigest = ""
	instance.pview = 0
	instance.pseq = 0
...
func (instance *pbftCore) witnessCheckpointWeakCert(chkpt *Checkpoint) {
	...
	if instance.skipInProgress {
		...
		// The view should not be set to active, this should be handled by the yet unimplemented SUSPECT, see https://github.com/cherryFloris/pbft-vrf/issues/1120
...
func (instance *pbftCore) startActiveDetectimer() {
	for i, t := range instance.activeDetectimers {
		t.Reset(instance.activeDetectimeout, activeDetectimeoutEvent{instance.peers[i]})
	}
}

func (instance *pbftCore) stopActiveDetectimer() {
	for _, t := range instance.activeDetectimers {
		t.Stop()
	}
}

func (instance *pbftCore) processPbftEventForDetectimer(e events.Event) {
	var peerID uint64 = 0
	switch et := e.(type) {
	case *PrePrepare:
		peerID = et.ReplicaId
	...
	if peerID != 0 {
		for i, peer := range instance.peers {
			if peer == peerID {
				instance.graph.ResetLinkWeight(instance.id, instance.peers[instance.peers[i]], weight)
				instance.activeDetectimers[i].Reset(instance.activeDetectimeout, activeDetectimeoutEvent{instance.peers[i]})
				break
			}
		}
	}
